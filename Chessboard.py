...
from component.Move import Move
from DLVConnector import DLVConnector
...
class Chessboard:
    ...
    #WHERE DLV MUST DO HIS JOB
    def ai_move(self):
        if not self.game_over:
            #write facts in DLVConnector
            facts = ""
            for r in range(self.grid_count):
                for c in range(self.grid_count):
                    if self.grid[r][c] == 'b':
                        facts += "Cell(" + str(r) + "," + str(c) + ",2).\n"
                    elif self.grid[r][c] == 'w':
                        facts += "Cell(" + str(r) + "," + str(c) + ",1).\n"
                    elif self.grid[r][c] == '.':
                        facts += "Cell(" + str(r) + "," + str(c) + ",0).\n"

            self.dlv.set_facts(facts)

            #setDLV
            self.dlv.set_DLV()

            #getSolution
            bestMove: str =  self.dlv.get_solution()

            #call dlv
            move = bestMove.split(',')
            move = Move(int(move[0]), int(move[1]))
            if self.grid[move.row][move.column] == '.':
                self.grid[move.row][move.column] = self.aiPiece
                self.check_win(move.row, move.column)
                return

    def set_piece(self, r, c):
        if self.grid[r][c] == '.':
            self.grid[r][c] = self.userPiece
            """
            if self.piece == 'b':
                self.piece = 'w'
            else:
